  ...
  return (
    ...
    >
      <form
        ...
        className="max-w-xl space-y-3 rounded-xl border border-[#E5E7EB] bg-white p-5 shadow-sm"
      >
      ...
      <section className="space-y-3">
        <h2 className="text-lg font-semibold text-[#111827]">Créneaux actifs</h2>
        <div className="grid gap-3 sm:grid-cols-3">
          {slots.map((slot) => (
            <div
              key={slot.id}
              className="rounded-xl border border-[#E5E7EB] bg-white p-4 text-sm shadow-sm"
            >
      ...
      <section className="space-y-3">
        <h2 className="text-lg font-semibold text-[#111827]">Réservations du mois</h2>
        {[...countByDay.entries()].length === 0 ? (
          <p className="text-[#6B7280]">Aucune réservation ce mois.</p>
        ) : (
          <div className="space-y-3">
            {[...countByDay.entries()].map(([date, count]) => (
              <div key={date} className="rounded-xl border border-[#E5E7EB] bg-white p-4 shadow-sm">
                <div className="text-sm text-[#6B7280]">{date}</div>
      ...
      <section className="space-y-3">
        <h2 className="text-lg font-semibold text-[#111827]">Paramètres RDV</h2>
      ...
      <section className="space-y-3">
        <h2 className="text-lg font-semibold text-[#111827]">Jours fériés</h2>
        <form
          action={upsertHolidayAction}
          className="flex max-w-xl items-end gap-3 rounded-xl border border-[#E5E7EB] bg-white p-4"
        >
          <div>
        ...
        {holidays.length === 0 ? (
          <p className="text-[#6B7280]">Aucun jour férié défini.</p>
        ) : (
          <div className="space-y-2">
            {holidays.map((h) => (
              ...
              >
                <div>
                  <div className="text-sm text-[#6B7280]">{h.date.toISOString().slice(0, 10)}</div>
