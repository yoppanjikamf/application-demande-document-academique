  ...
  return (
    ...
    >
      <form
        ...
        className="max-w-xl space-y-3 rounded-md border border-[var(--border-token)] bg-surface-0 p-5 shadow-card"
      >
      ...
      <section className="space-y-3">
        <h2 className="text-lg font-semibold text-text-1">Créneaux actifs</h2>
        <div className="grid gap-3 sm:grid-cols-3">
          {slots.map((slot) => (
            <div
              key={slot.id}
              className="rounded-md border border-[var(--border-token)] bg-surface-0 p-4 text-sm shadow-card"
            >
      ...
      <section className="space-y-3">
        <h2 className="text-lg font-semibold text-text-1">Réservations du mois</h2>
        {[...countByDay.entries()].length === 0 ? (
          <p className="text-text-3">Aucune réservation ce mois.</p>
        ) : (
      ...
      <section className="space-y-3">
        ...
        {holidays.length === 0 ? (
          <p className="text-text-3">Aucun jour férié défini.</p>
        ) : (
          <div className="space-y-2">
            {holidays.map((h) => (
              <div
                key={h.id}
                className="flex items-center justify-between rounded-md border border-[var(--border-token)] bg-surface-0 p-3"
              >
                <div>
                  <div className="text-sm text-text-3">{h.date.toISOString().slice(0, 10)}</div>
