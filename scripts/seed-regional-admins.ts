...
import { OBC_REGIONAL_ANTENNAS, ORGANISME_IDS, REGIONAL_ANTENNAS } from "../lib/document-routing";
...
const DOC_PATH = path.join(process.cwd(), "docs", "admins-regionaux-test.md");
...
function buildRegionalAdmins(): RegionalAdminSeed[] {
  return OBC_REGIONAL_ANTENNAS.map((antenne, index) => {
    ...
    return {
      ...
      email: normalizeEmail(`admin.${regionSlug}@example.com`),
      password: `Admin${regionCompact}2026!`,
      matricule: normalizeMatricule(`ADM-${String(index + 1).padStart(2, "0")}-${regionSlug}`),
      nom: "Admin",
      prenom: antenne.region,
      nomService: `OBC ${antenne.region}`,
    };
  });
}
...
async function writeCredentialsDoc(admins: RegionalAdminSeed[]) {
  ...
  const content = `# Admins regionaux de test
...
async function main() {
  ...
  for (const input of admins) {
    ...
    console.log(`Admin ${input.region} pret: ${input.email} (${input.matricule})`);
  }
